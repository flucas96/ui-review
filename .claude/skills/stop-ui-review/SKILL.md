...
- Resolve the project root and read `.ui-review/session.json`.
- If no session record exists, report that no managed session was found. Do not guess with broad `pkill`, `killall`, or port-based termination.
...
- Verify that managed processes exited, then remove `session.json` and stale PID metadata.
