...
- If no per-session record exists, inspect legacy `.ui-review/session.json` only as a backward-compatible fallback. Never guess with broad `pkill`, `killall`, or port-based termination.
